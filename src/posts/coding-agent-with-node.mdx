...
This last year has been a wild ride in the AI coding space, especially with the release of tools like Claude Code and the explosion of CLI Coding Agents. What blows my mind is how simple they actually are once you understand the core idea. In this post, I will try to demystify them by building your own coding agent with JavaScript. I encourage you to follow along!
...
On Node.js, we can use the **_readline_** module as a straightforward way to create an input/chat loop. **_Readline_** lets us read data from any readable stream one line at a time, such as the terminal’s standard input. Pretty neat!

To configure it, we first need to create an interface to define the input and output (in this case, the terminal), then use it to prompt the user for input with the `question` method. With this code, we have a chat that responds with the same thing we introduce and saves all messages.
...
export async function chat() {
	while (true) {
		...
		// We will save the role of who sent the mesage, this will be important later
...
> [!NOTE]
> Now is a good time to run `npm init`, since we’ll need a `package.json` file later. You can use `npm init -y` to skip all the prompts.
...
Agent: Hi! I’m **Ministral-3-8B-Instruct**, a large language model developed by **Mistral AI** (a French AI lab based in Paris). Think of me as your friendly AI assistant—here to help with questions, explanations, brainstorming, or just chatting!
...
So, what are tools really? They are concrete pieces of code that execute when the Agent says so and with the input it sees fit.

Tools bridge the gap between what an LLM can do (generate text) and what we need it to do. Here’s the dance we have to perform to make that happen: the agent writes a request for a tool, we execute it, and we send back the result as plain text.

To make this work, we pass the agent a list of available tools, including their names, what they do, and the inputs they expect. Modern LLMs are trained to call those tools eagerly if they apply to the task at hand. When the agent requests a tool, it sends a special message in the chat. We detect that message, run the function, and add the result back to the conversation.

Think of it this way: the agent can’t read files or write code directly. But it can ask you to do those things in a structured way. You execute the request, report back, and the agent uses that information to decide what to do next.

### What do we need for our coding agent?

What makes a coding agent tick is giving it 3 main types of tools: _search, read,_ and _write._

Digging a bit deeper, the tools we would need here are:

- _Find_ - To find files or with glob patterns
- _Grep_ - To find code
- _Read_ - To read files
- _Create_ - To create new files
- _Edit_ - To edit an existing file

One of the cool things about the tools we give the agent is that they are closely tied to language, which is what the LLM really understands. I told you before that you could think of tools as normal functions in code. It’s very close, but you might not get everything out of the model if you’re purely thinking like a programmer. Instead, write things closer to how they would read in plain English. Think of tools as something you would tell someone who needs to know exactly what to do, not necessarily as writing a traditional computer program.
...
We include descriptions throughout to help the agent understand when and how to use each one. However, this leaves us with the final piece missing—the "glue." Let’s finish this!
...
```javascript title="index.js" collapse={1-53,} ins={34-164}
import { styleText } from 'node:util'
import * as readline from 'node:readline/promises'
import ollama from 'ollama'
import { exec } from 'child_process'
...
And that’s it. We have our own coding agent in 300 lines of code!

# The test drive

Now we can try it ourselves to see what it does. Let’s ask him to describe the project.
...
2. **A general project** you’ve mentioned earlier in our conversation?
...
This isn’t a perfect system prompt, but watch how it transforms the agent’s behavior. If we ask the same question again, the agent responds more consistently and much better.

```javascript
You: could you describe me this project?
Agent:
[Tool Call: findFiles] inputs: {"pattern":"README*"}
Agent:
[Tool Call: findFiles] inputs: {"pattern":".gitignore"}
Agent:
[Tool Call: readFile] inputs: {"filePath":".gitignore"}
Agent:
[Tool Call: findFiles] inputs: {"pattern":"package.json"}
Agent:
[Tool Call: readFile] inputs: {"filePath":"package.json"}
...
I challenge you to improve this basic coding agent using the agent itself—the program programming itself!

Try implementing a to-do list tool for the agent without writing any code manually. Give it a try and see what it can do. You will see how he thinks and be able to tweak how it uses its tools and why.
